-- Add IP scopes to datacenters table
ALTER TABLE datacenters ADD COLUMN ip_scopes JSONB DEFAULT '[]'::jsonb;

-- Add comments for clarity
COMMENT ON COLUMN datacenters.ip_scopes IS 'Array of IP scope objects: [{"subnet": "192.168.1.0/24", "vlan": 100, "description": "Production Network"}]';

-- Create function to check if IP is in datacenter scope
...
BEGIN
    ...
    FOR dc_id IN 
        SELECT id FROM datacenters WHERE is_active = true
    LOOP
...
BEGIN
    ...
    assigned_dc_id := get_datacenter_for_ip(NEW.ip_address);
    
    -- If found and not already set, assign it
    IF assigned_dc_id IS NOT NULL THEN
        NEW.datacenter := (SELECT name FROM datacenters WHERE id = assigned_dc_id);
    END IF;
...
DROP TRIGGER IF EXISTS trigger_auto_assign_datacenter ON servers;
CREATE TRIGGER trigger_auto_assign_datacenter
    BEFORE INSERT OR UPDATE OF ip_address ON servers
    FOR EACH ROW
    EXECUTE FUNCTION auto_assign_datacenter();