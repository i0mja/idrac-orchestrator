...
function Install-Application {
    ...
    $distSource = if ($projectRoot) { Join-Path $projectRoot "dist" } else { $null }

    New-Item -ItemType Directory -Path $InstallPath -Force | Out-Null

    if ($distSource -and (Test-Path $distSource)) {
        Copy-Item -Path $distSource -Destination $InstallPath -Recurse -Force
    } else {
        Write-Warning "Build artifacts not found locally. Downloading pre-built bundle..."
        $tempZip = Join-Path $env:TEMP "idrac-orchestrator-dist.zip"
        $distUrl = "https://github.com/i0mja/idrac-orchestrator/releases/latest/download/dist.zip"
        try {
            ...
            Write-Success "Downloaded and extracted pre-built bundle"
        } catch {
            Write-Error "Could not obtain build artifacts. Run 'npm run build' locally and re-run installer."
            exit 1
        }
    }

    $serveTarget = Join-Path $InstallPath 'serve.js'
    Invoke-WebRequest -Uri "https://raw.githubusercontent.com/i0mja/idrac-orchestrator/main/server/serve.js" -OutFile $serveTarget -UseBasicParsing
