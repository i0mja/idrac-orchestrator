  ...
  return {
    async enterMaintenance(hostMoid: string, opts: { evacuatePoweredOffVMs: boolean; timeoutMinutes: number }) {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ evacuate_all: true, timeout: Math.max(60, (opts.timeoutMinutes ?? 60) * 60) })
      });
      if (!res.ok) throw new Error('enterMaintenance failed');
      return { taskId: (await res.json())?.value ?? '' };
    },

    async exitMaintenance(hostMoid: string) {
      const res = await fetch(`${baseUrl}/rest/vcenter/host/maintenance-mode/${hostMoid}?action=exit`, {
        method: 'POST',
        headers
      });
    ...
    // Test-friendly: allow custom fetch impl injection
